...
[^2]: All services offer a free tier with limited amount of requests per month. The user needs provide suitable account credentials. A quick start description how to do it is available at the [wiki pages](https://github.com/grp-bork/formHTR/wiki/Setup-services) of the repository.

Next step is a binning of identified contents into the ROIs for each service. An appropriate ROI needs to be decided for each captured fragment of text (a word). There are several cases that need to be considered, such as a word spanning over multiple ROIs or multiple words overlapping with a single ROI (for examples, see \autoref{fig:binning}). To find all overlaps for a ROI from all the services, the number of possibilities and cases that need to be investigated can grow. We use ```R-tree``` [@rtree] to index the regions and capture their overlaps for all services' outputs and the template specification. The ```R-tree``` consequently allows an effective querying for intersections. Additionally, the logsheet can contain preprinted contents (*residuals*) that do not belong to ROIs, but can be shifted to its boundary box by an imperfect alignment. This is handled by defining the residuals already in the annotation step, allowing their elimination from the outputs.

![Examples of various situations of the text position identification w.r.t a ROI. (PLACEHOLDER) \label{fig:binning}](binning.png)
...
An example workflow comprises of the three steps, assuming we have a template and scanned logsheet. We first need to create and annotate ROIs for the template, and consequently process the logsheet, creating the output Excel spreadsheet.
